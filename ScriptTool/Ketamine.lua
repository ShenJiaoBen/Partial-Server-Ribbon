...
do -- Set properties
    ...
    objects["Instance68"]["Text"] = "HTTP请求";
...
return {
    { "1.0", "氯胺酮被发布为CSpy（樱桃间谍），具有简单的设置、远程和事件间谍功能." },
    { "1.1", "CSpy更名为氯胺酮，增加了脚本扫描和http间谍。" },
    { "1.2", "修复了由脚本扫描程序引起的延迟，提高了其速度，增加了在远程和事件间谍中钩住参数的能力。" },
    { "1.3", "添加了更改日志，修复了坏执行器上的http间谍错误（不是致命的），添加了内存扫描程序。<b>现在不可用的页面，如内存扫描程序，在不受支持的执行器上不可见。_中文汉化来自Yuxin</b>" }
}
    end;
...
return function(shared, page)
    ...
    addButton("阻止远程[对象]", function()
        if selection[1] then
            block[selection[1]] = true
        end
    end)
    addButton("阻止远程[名称]", function()
        if selection[1] then
            block[selection[1].Name] = true
        end
    end)
    
    addButton("清除记录", function()
        for _, log in logs do
            log:Destroy()
        end
    ...
    addButton("忽略远程[对象]", function()
        if selection[1] then
            ignore[selection[1]] = true
        end
    end)
    addButton("忽略远程[名称]", function()
        if selection[1] then
            ignore[selection[1].Name] = true
        end
    end)

    addButton("反编译", function()
        if selection[4] then
    ...
    addButton("钩子参数[对象]", function(button)
    ...
    addButton("钩子参数[名称]", function(button)
        if hooking[1] and hooking[1] ~= button.Name then return end
...
return function(shared, page)
    ...
    local clipboard = shared.Clipboard
    addButton("将文本复制到剪贴板", function()
        clipboard(codeBox.Text)
    end)

    addButton("将url复制到剪贴板", function()
        if selection[1] then
